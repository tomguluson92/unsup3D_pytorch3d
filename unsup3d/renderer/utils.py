...
def get_face_idx(b, h, w):
    """
        transplant this function to pytorch3d version.
    :param b:
    :param h:
    :param w:
    :return:
    ...
    faces1 = torch.stack([idx_map[:h-1,:w-1], idx_map[1:,:w-1], idx_map[:h-1,1:]], -1).reshape(-1,1)
    faces2 = torch.stack([idx_map[:h-1,1:], idx_map[1:,:w-1], idx_map[1:,1:]], -1).reshape(-1,1)
    return F.interpolate(torch.cat([faces1,faces2], 0).repeat(b,1,1).reshape(b,1,-1).float(), h*w).reshape(b,-1,1)
...
def get_textures_from_im(im, tx_size=1):
    ...
    if tx_size == 1:
        ...
        textures = textures.transpose(2,1).reshape(b,-1,c)
    elif tx_size == 2:
        # todo: only tx_size=1 is supported now.
